...
class SubscribeSearchItemListDiffer {
    ...
    val adapter: BaseRecyclerViewAdapter by lazy {
        object : BaseRecyclerViewAdapter(provider) {
            override fun onCreateItemViewHolder(
                ...
                return SubscribeItemViewHolder(parent)
            }
        }.apply {
    ...
    fun snapList(): List<ListAddress> {
        val newList = mutableListOf<ListAddress>()
        newList.addAll(dataSet.filterIsInstance<ListAddress>())
        return newList
    }

    fun setClearCheck(){
        try {
            dataSet.filterIsInstance<ListAddress>().forEach {
                it.isChecked = false
            }
        }catch (e: Exception) {}
    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val subscribeSearchItemListDifferCallback = SubscribeSearchItemListDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(subscribeSearchItemListDifferCallback)
    ...
    private fun createItemCallback(): DiffUtil.ItemCallback<ListAddress> {
        return object : DiffUtil.ItemCallback<ListAddress>() {
...
class SubscribeSearchItemListDifferCallback(
