...
class SearchFlexItemTwoListDiffer {
    ...
    val adapter: BaseRecyclerViewAdapter by lazy {
        object : BaseRecyclerViewAdapter(provider) {
            override fun onCreateItemViewHolder(
                ...
                return SearchFlexItemViewHolder(parent)
            }
        }.apply {
    ...
    fun snapList(): List<Allergy> {
        val newList = mutableListOf<Allergy>()
        newList.addAll(dataSet.filterIsInstance<Allergy>())
        return newList
    }

    private fun calDiff(newTiles: List<Any>) {
        val searchFlexTwoDiffUtilCallback = SearchFlexTwoDiffUtilCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(searchFlexTwoDiffUtilCallback)
    ...
    private fun createItemCallback(): DiffUtil.ItemCallback<Allergy> {
        return object : DiffUtil.ItemCallback<Allergy>() {
...
class SearchFlexTwoDiffUtilCallback(
