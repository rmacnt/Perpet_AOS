...
import pet.perpet.equal.presentation.sign.viewholder.DiseaseViewHolder
...
class PackageItemListDiffer  {
            ...
            override val holderSet: ViewHolderSet2<Any> by lazy {
                object : ViewHolderSet2<Any>() {
                    override fun onDependencyViewHolder(key: Any?): Class<out RecyclerViewHolder<*>> {
                        if (key is String) {
                            return PackageListItemViewHolder::class.java
                        }
                        return PackageListItemViewHolder::class.java
                    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val packageItemListDifferCallback = PackageItemListDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(packageItemListDifferCallback)
...
class PackageItemListDifferCallback(
