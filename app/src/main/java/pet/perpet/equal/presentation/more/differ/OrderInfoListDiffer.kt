package pet.perpet.equal.presentation.more.differ
...
class OrderInfoListDiffer {
    ...
    val adapter: BaseRecyclerViewAdapter by lazy {
        object : BaseRecyclerViewAdapter(provider) {
            override fun onCreateItemViewHolder(
                ...
                return OrderInfoViewHolder(parent)
            }
        }.apply {
    ...
    fun snapList(): List<OrderInfo> {
        val newList = mutableListOf<OrderInfo>()
        newList.addAll(dataSet.filterIsInstance<OrderInfo>())
        return newList
    }

    private fun calDiff(newTiles: List<Any>) {
        val orderInfoListDifferCallback = OrderInfoListDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(orderInfoListDifferCallback)
    ...
    private fun createItemCallback(): DiffUtil.ItemCallback<String> {
...
class OrderInfoListDifferCallback(
