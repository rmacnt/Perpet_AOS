...
import pet.perpet.equal.presentation.more.fragment.viewholder.ItemAddressViewHolder
...
class AddressItemListDiffer {
            ...
            override val holderSet: ViewHolderSet2<Any> by lazy {
                object : ViewHolderSet2<Any>() {
                    override fun onDependencyViewHolder(key: Any?): Class<out RecyclerViewHolder<*>> {
                        if (key is String) {
                            return ItemAddressViewHolder::class.java
                        }
                        return ItemAddressViewHolder::class.java
                    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val addressItemListDifferCallback = AddressItemListDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(addressItemListDifferCallback)
...
class AddressItemListDifferCallback(
