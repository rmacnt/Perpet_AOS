...
import pet.perpet.equal.presentation.supplement.viewholder.CodeImageViewHolder
...
class CodeImageItemListDiffer {
            ...
            override val holderSet: ViewHolderSet2<Any> by lazy {
                object : ViewHolderSet2<Any>() {
                    override fun onDependencyViewHolder(key: Any?): Class<out RecyclerViewHolder<*>> {
                        if (key is String) {
                            return CodeImageViewHolder::class.java
                        }
                        return CodeImageViewHolder::class.java
                    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val codeImageItemListDifferCallback = CodeImageItemListDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(codeImageItemListDifferCallback)
...
class CodeImageItemListDifferCallback(
