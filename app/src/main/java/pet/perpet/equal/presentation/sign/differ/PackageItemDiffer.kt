...
import pet.perpet.equal.presentation.subscribe.viewholder.PackageItemViewHolder
...
class PackageItemDiffer {
            ...
            override val holderSet: ViewHolderSet2<Any> by lazy {
                object : ViewHolderSet2<Any>() {
                    override fun onDependencyViewHolder(key: Any?): Class<out RecyclerViewHolder<*>> {
                        if (key is String) {
                            return PackageItemViewHolder::class.java
                        }
                        return PackageItemViewHolder::class.java
                    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val packageItemDifferCallback = PackageItemDifferCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(packageItemDifferCallback)
...
class PackageItemDifferCallback(
