...
class SearchAllergyListDiffer {
    ...
    val adapter: BaseRecyclerViewAdapter by lazy {
        object : BaseRecyclerViewAdapter(provider) {
            override fun onCreateItemViewHolder(
                ...
                return AllergySearchViewHolder(parent)
            }
        }.apply {
    ...
    fun setClearCheck(){
        try {
            dataSet.filterIsInstance<Allergy>().forEach {
                it.isChecked = false
            }
        }catch (e: Exception) {}
    }
    ...
    private fun calDiff(newTiles: List<Any>) {
        val searchAllergyDiffUtilCallback = SearchAllergyDiffUtilCallback(dataSet, newTiles)
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(searchAllergyDiffUtilCallback)
...
class SearchAllergyDiffUtilCallback(
