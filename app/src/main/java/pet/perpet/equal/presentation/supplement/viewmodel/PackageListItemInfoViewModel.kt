...
open class PackageListItemInfoViewModel (var model: Package? = null) {
    ...
    val count: String?
        get() = getString(R.string.subscribe_comment53)?.let { String.format(it,
            DecimalFormat("#,###").format((model?.rxInfo?.daily_dosage_mg.nonnull()) )) }
