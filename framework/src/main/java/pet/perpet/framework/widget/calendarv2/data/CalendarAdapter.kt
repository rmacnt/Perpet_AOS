        ...
        for (i in offset until length + offset) {
            ...
            for (j in mEventList.indices) {
                ...
                if (day.year == event.year
                    ...
                    if(event.color == 1) {
                        txtDay.setTextColor(ContextCompat.getColor(txtDay.context , R.color.calendar_color))
                        imgEventTag.setImageResource(R.drawable.m_circle_one)
                    } else {
                        txtDay.setTextColor(ContextCompat.getColor(txtDay.context , R.color.black))
                        imgEventTag.setImageResource(R.drawable.m_circle_two)
                    }
                } else {
                    if (day.year == event.year
                        && day.month != event.month) {
                        imgEventTag.visibility = View.GONE
                    }
                    txtDay.setTextColor(ContextCompat.getColor(txtDay.context , R.color.black))
